...
class CRUDService(Service):
    ...
    def create(self, obj_data: dict | ModelT) -> ModelT:
        with self.uow:
    ...
    def update(self, obj_data: dict | ModelT, *filters, **kwargs_filters) -> ModelT:
        with self.uow:
            if isinstance(obj_data, dict):
    ...
    def list(self, *filters, lazy: bool = False, **kwargs_filters) -> Iterable[ModelT] | LazyCommand[Iterable[ModelT]]:
        return self.uow.repository.filter(self._specs.filter(*filters, **kwargs_filters), lazy=lazy)

    def get(self, *filters, lazy: bool = False, **kwargs_filters) -> ModelT | LazyCommand[ModelT]:
